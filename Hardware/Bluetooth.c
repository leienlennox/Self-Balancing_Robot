...
uint8_t Bluetooth_RxFlag;					//定义接收数据包标志位
uint8_t RxData;
...
void Bluetooth_SendArray(uint8_t *Array, uint16_t Length)
{
	...
	{
		Bluetooth_SendByte(Array[i]);		//依次调用Bluetooth_SendByte发送每个字节数据
	}
}
...
void Bluetooth_SendString(char *String)
{
	...
	{
		Bluetooth_SendByte(String[i]);		//依次调用Bluetooth_SendByte发送每个字节数据
	}
}
...
void Bluetooth_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	{
		Bluetooth_SendByte(Number / Bluetooth_Pow(10, Length - i - 1) % 10 + '0');	//依次调用Bluetooth_SendByte发送每位数字
	}
}
...
void USART1_IRQHandler(void)
{
	...
	if (USART_GetITStatus(USART1, USART_IT_RXNE) == SET)	//判断是否是USART1的接收事件触发的中断
	{
		...
		if (RxState == 0)
		{
			if (RxData == '@' && Bluetooth_RxFlag == 0)		//如果数据确实是包头，并且上一个数据包已处理完毕
			{
				RxState = 1;			//置下一个状态
		...
		else if (RxState == 1)
		{
			...
			{
				Bluetooth_RxPacket[pRxPacket] = RxData;		//将数据存入数据包数组的指定位置
				pRxPacket ++;								//数据包的位置自增
			}
		}
		/*当前状态为2，接收数据包第二个包尾*/
		else if (RxState == 2)
		{
			...
			{
				RxState = 0;			//状态归0
				Bluetooth_RxPacket[pRxPacket] = '\0';			//将收到的字符数据包添加一个字符串结束标志
				Bluetooth_RxFlag = 1;		//接收数据包标志位置1，成功接收一个数据包
			}
		}
